...
cmdline_read_file(
	...
	{
	struct file			fil;
	cw_char_t			data[GLOBAL_MAX_CONFIG_SIZE];
	cw_size_t			size;

	if (! file_open(&fil, path, FILE_MODE_READ, flags)) return;
	size = file_read(&fil, data, sizeof (data));
...
cmdline_read_rc_files(
	...
	cmdline_read_file("/etc/cwtoolrc", FILE_FLAG_RETURN);
	home = getenv("HOME");
	if (home == NULL) return;
	string_snprintf(path, sizeof (path), "%s/.cwtoolrc", home);
	cmdline_read_file(path, FILE_FLAG_RETURN);
	}
...
cmdline_get_file(
	...
	{
	if ((i < 0) || (i >= cmd.files)) return (NULL);
	return (cmd.file[i]);
	}
...
cmdline_read_config(
	...
	for (e = i = 0; i < cmd.configs; i++)
		...
		if (cmd.cfg[i].type == CMDLINE_CONFIG_TYPE_FILE)
			{
			cmdline_read_file(cmd.cfg[i].data, FILE_FLAG_NONE);
			continue;
			}
		string_snprintf(number, sizeof (number), "-e/--evaluate #%d", ++e);
		config_parse(number, cmd.cfg[i].data, string_length(cmd.cfg[i].data));
		}
