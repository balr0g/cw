...
gcr_apple_statistics(
	...
	if (fmt->gcr_apl.rd.flags & FLAG_POSTCOMP_SIMPLE) histogram_postcomp_simple(ffo_l0, fmt->gcr_apl.rw.bnd, 3, track, xlated_track);
	return (1);
	}
...
gcr_apple_read_track2(
	...
	while (gcr_apple_read_sector(&ffo_l1, &fmt->gcr_apl, con, dsk_sct, track) != -1) ;
	}
...
gcr_apple_read_track(
	...
	else gcr_apple_read_track2(fmt, NULL, ffo_l0, ffo_l3, dsk_sct, track);
	return (1);
	}
