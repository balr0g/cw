...
mfm_amiga_statistics(
	...
	if (fmt->mfm_amg.rd.flags & FLAG_POSTCOMP_SIMPLE) histogram_postcomp_simple(ffo_l0, fmt->mfm_amg.rw.bnd, 3, track, track);
	return (1);
	}
...
mfm_amiga_read_track2(
	...
	while (mfm_amiga_read_sector(&ffo_l1, &fmt->mfm_amg, con, dsk_sct, track) != -1) ;
	}
...
mfm_amiga_read_track(
	...
	else mfm_amiga_read_track2(fmt, NULL, ffo_l0, ffo_l3, dsk_sct, track);
	return (1);
	}
