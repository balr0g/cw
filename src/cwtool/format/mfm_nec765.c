...
mfm_nec765_statistics(
	...
	{
	histogram_normal(ffo_l0, track, mfm_nec765_track_number(&fmt->mfm_nec, track));
	if (fmt->mfm_nec.rd.flags & FLAG_RD_POSTCOMP_SIMPLE) histogram_postcomp_simple(ffo_l0, fmt->mfm_nec.rw.bnd, 3, track, mfm_nec765_track_number(&fmt->mfm_nec, track));
	return (1);
	}
...
mfm_nec765_read_track2(
	...
	while (mfm_nec765_read_sector(&ffo_l1, &fmt->mfm_nec, con, dsk_sct, track) != -1) ;
	}
...
mfm_nec765_read_track(
	...
	else mfm_nec765_read_track2(fmt, NULL, ffo_l0, ffo_l3, dsk_sct, track);
	return (1);
	}
