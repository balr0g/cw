...
gcr_cbm_write_sector(
	...
	verbose(1, "writing sector %d with id 0x%02x 0x%02x", sector, id[0], id[1]);
...
gcr_cbm_statistics(
	...
	{
	histogram_normal(ffo_l0, track, gcr_cbm_track_number(&fmt->gcr_cbm, track));
	if (fmt->gcr_cbm.rd.flags & FLAG_POSTCOMP_SIMPLE) histogram_postcomp_simple(ffo_l0, fmt->gcr_cbm.rw.bnd, 3, track, gcr_cbm_track_number(&fmt->gcr_cbm, track));
	return (1);
	}
...
gcr_cbm_read_track2(
	...
	while (gcr_cbm_read_sector(&ffo_l1, &fmt->gcr_cbm, con, dsk_sct, track) != -1) ;
	}
...
gcr_cbm_read_track(
	...
	else gcr_cbm_read_track2(fmt, NULL, ffo_l0, ffo_l3, dsk_sct, track);
	return (1);
	}
...
gcr_cbm_write_track(
	...
	for (i = 0; i < fmt->gcr_cbm.rw.sectors; i++) if (gcr_cbm_write_sector(&ffo_l1, &fmt->gcr_cbm, &dsk_sct[i], id, track) == -1) return (0);
