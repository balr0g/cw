...
container_store_data_and_error(
	...
	if (i >= CONTAINER_NR_ENTRIES) error_internal();
...
container_get_data(
	...
	{
	if ((index < 0) || (index >= con->entries)) error_internal();
...
container_set_limit(
	...
	{
	if ((index < 0) || (index >= con->entries)) error_internal();
	if ((limit < 0) || (limit > con->size[index])) error_internal();
...
container_store_range_sector(
	...
	if (i >= CONTAINER_NR_RANGES) error_internal();
...
container_get_range_sector(
	...
	{
	if ((index < 0) || (index >= con->entries)) error_internal();
	if ((range_index < 0) || (range_index >= con->range_entries[index])) error_internal();
