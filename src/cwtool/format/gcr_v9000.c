...
gcr_v9000_read_sector(
	...
	sector = header[2];
	track  = gcr_v9000_track_number(gcr_v9, track);
...
gcr_v9000_write_sector(
	...
	header[1] = gcr_v9000_track_number(gcr_v9, track);
	header[2] = sector;
...
gcr_v9000_statistics(
	...
	{
	histogram_normal(ffo_l0, track, gcr_v9000_track_number(&fmt->gcr_v9, track));
	if (fmt->gcr_v9.rd.flags & FLAG_RD_POSTCOMP_SIMPLE) histogram_postcomp_simple(ffo_l0, fmt->gcr_v9.rw.bnd, 3, track, gcr_v9000_track_number(&fmt->gcr_v9, track));
/*	histogram_analyze_gcr(ffo_l0, fmt->gcr_v9.rw.bnd, 3, track, gcr_v9000_track_number(&fmt->gcr_v9, track));*/
	return (1);
	}
...
gcr_v9000_read_track2(
	...
	raw_read(ffo_l0, &ffo_l1, fmt->gcr_v9.rw.bnd, 3);
	while (gcr_v9000_read_sector(&ffo_l1, &fmt->gcr_v9, con, dsk_sct, track) != -1) ;
	}
...
gcr_v9000_read_track(
	...
	else gcr_v9000_read_track2(fmt, NULL, ffo_l0, ffo_l3, dsk_sct, track);
	return (1);
	}
...
gcr_v9000_write_track(
	...
	for (i = 0; i < fmt->gcr_v9.rw.sectors; i++) if (gcr_v9000_write_sector(&ffo_l1, &fmt->gcr_v9, &dsk_sct[i], track) == -1) return (0);
	...
	if (raw_write(&ffo_l1, ffo_l0, fmt->gcr_v9.rw.bnd, fmt->gcr_v9.wr.precomp, 3) == -1) return (0);
	return (1);
	}
...
#define MAGIC_BOUNDS_OLD		19
#define MAGIC_BOUNDS_NEW		20
...
static struct format_option		gcr_v9000_rw_options[] =
	...
	FORMAT_OPTION_BOOLEAN("flip_track_id", MAGIC_FLIP_TRACK_ID, 1),
