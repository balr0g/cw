...
histogram_get_max(
	...
	if ((GLOBAL_NR_PULSE_LENGTHS % increment) != 0) error_internal();
...
histogram_head_line(
	...
	l = string_snprintf(line, size, "===[track %d (%d)", track, xlated_track);
...
histogram_long2(
	...
	printf("%s\n", histogram_head_line(line, sizeof (line), 128 + 6, track, xlated_track, info, info2));
	for (i = 20; i-- > 0; )
		{
...
histogram_short2(
	...
	printf("%s\n", histogram_head_line(line, sizeof (line), 64 + 6, track, xlated_track, info, info2));
	for (i = 20; i-- > 0; )
		{
		m = histogram_get_scale(i, m, max);
		printf("%5d ", m);
...
histogram_normal2(
	...
	cw_void_t			(*func)(cw_hist_t, cw_count_t, cw_count_t, const cw_char_t *, const cw_char_t *);
	cw_index_t			i;

	func = histogram_short2;
	if (verbose_level >= -2) func = histogram_long2;
	histogram_calculate(data, size, histogram, histogram2);
	func(histogram, track, xlated_track, info, NULL);
	...
	for (i = 0; i < GLOBAL_NR_PULSE_LENGTHS; i++)
		...
		func(histogram2[i], track, xlated_track, info, info2);
		}
	}
...
histogram_long(
	...
	{
	histogram_long2(histogram, track, xlated_track, NULL, NULL);
	}
...
histogram_short(
	...
	{
	histogram_short2(histogram, track, xlated_track, NULL, NULL);
	}
...
histogram_normal(
	...
	{
	histogram_normal2(ffo, track, xlated_track, NULL);
	}
...
histogram_postcomp_simple(
	...
	if (verbose_level >= -1)
		...
		histogram_normal2(ffo, track, xlated_track, info);
		}
	}
...
histogram_analyze_gcr(
	...
	{
	histogram_analyze(ffo, bnd, bnd_size, track, xlated_track, 1);
	}
...
histogram_analyze_mfm(
	...
	{
	histogram_analyze(ffo, bnd, bnd_size, track, xlated_track, 2);
	}
/******************************************************** Karsten Scheibler */
