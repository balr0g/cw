...
image_d64_read(
	...
	verbose(1, "reading D64 track %d with %d bytes from '%s'", track, size, file_get_path(&img->d64.fil));
...
image_d64_write(
	...
	verbose(1, "writing D64 track %d with %d bytes to '%s'", track, size, file_get_path(&img->d64.fil));
