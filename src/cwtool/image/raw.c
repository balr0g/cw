...
/****************************************************************************
 * image_raw_text_error
 ****************************************************************************/
static cw_void_t
image_raw_text_error(
	struct image_raw_text		*img_raw_txt,
	struct file			*fil,
	const cw_char_t			*msg)
...
image_raw_text_get_char(
	...
	cw_char_t			c;
	cw_index_t			i;
	static const cw_char_t		valid[] = "\n\r\t _{}#$\"";

	if (img_raw_txt->ofs >= img_raw_txt->limit)
		{
		img_raw_txt->limit = file_read(fil, img_raw_txt->text, img_raw_txt->size);
		img_raw_txt->ofs = 0;
		if (img_raw_txt->limit == 0) return ('\0');
		}
	c = img_raw_txt->text[img_raw_txt->ofs++];
	if (c == '\0') image_raw_text_error(img_raw_txt, fil, "got \\0 character");
	if (allow_all) goto ok;
	if ((c >= '0') && (c <= '9')) goto ok;
	if ((c >= 'A') && (c <= 'Z')) goto ok;
	if ((c >= 'a') && (c <= 'z')) goto ok;
	for (i = 0; valid[i] != '\0'; i++) if (c == valid[i]) goto ok;
	image_raw_text_error(img_raw_txt, fil, "illegal character");
ok:
	img_raw_txt->line_ofs++;
	if (c == '\n') img_raw_txt->line++, img_raw_txt->line_ofs = 0;
	return (c);
	}
...
image_raw_text_number_hex(
	...
	while (*token != '\0')
		...
		else image_raw_text_error(img_raw_txt, fil, "not a hexadecimal number");
		if (num > (0x7fffffff - 15) / 16) image_raw_text_error(img_raw_txt, fil, "hexadecimal number too large");
		num = 16 * num + c;
		if (num > 255) image_raw_text_error(img_raw_txt, fil, "hexadecimal number too large");
		}
...
image_raw_text_number_dec(
	...
	while (*token != '\0')
		{
		c = *token++;
		if ((c < '0') || (c > '9')) image_raw_text_error(img_raw_txt, fil, "not a decimal number");
		if (num > (0x7fffffff - 9) / 10) image_raw_text_error(img_raw_txt, fil, "decimal number too large");
		num = 10 * num + c - '0';
		if (num > 255) image_raw_text_error(img_raw_txt, fil, "decimal number too large");
		}
	debug_error_condition(num < 0);
	return (num);
	}
...
image_raw_text_token(
	...
	for (comment = CW_BOOL_FALSE, i = 0, token[0] = '\0'; i < size - 1; )
		{
		c = image_raw_text_get_char(img_raw_txt, fil, comment);
		if (image_raw_text_comment(c, &comment)) continue;
		if ((c == '\0') || (image_raw_text_is_space(c)))
			{
			if ((c != '\0') && (i == 0)) continue;
	...
	image_raw_text_error(img_raw_txt, fil, "token too long");
...
image_raw_text_number(
	...
	if (token == NULL)
		{
		size  = image_raw_text_token(
			img_raw_txt,
			fil,
			token2,
			GLOBAL_MAX_NAME_SIZE);
		token = token2;
		}
	if (size <= 0) image_raw_text_error(img_raw_txt, fil, "number expected");
	if (hex_only) num = image_raw_text_number_hex(img_raw_txt, fil, token);
	else if ((size > 2) && (token[0] == '0') && (token[1] == 'x')) num = image_raw_text_number_hex(img_raw_txt, fil, &token[2]);
	else if ((size > 1) && (token[0] == '$')) num = image_raw_text_number_hex(img_raw_txt, fil, &token[1]);
	else num = image_raw_text_number_dec(img_raw_txt, fil, token);
...
image_raw_ioctl(
	...
	if (tri.clock >= img_raw->fli.nr_clocks) error("error while accessing track %d, clock is not supported by device '%s'", track, file_get_path(&img_raw->fil[0]));
	if (tri.track >= img_raw->fli.nr_tracks) error("error while accessing track %d, track is not supported by device '%s'", track, file_get_path(&img_raw->fil[0]));
	if (tri.side  >= img_raw->fli.nr_sides)  error("error while accessing track %d, side is not supported by device '%s'", track, file_get_path(&img_raw->fil[0]));
	if (tri.mode  >= img_raw->fli.nr_modes)  error("error while accessing track %d, mode is not supported by device '%s'", track, file_get_path(&img_raw->fil[0]));
...
image_raw_read_correction(
	...
	verbose(1, "doing '0x80 -> 0x7f' correction on raw track %d", track);
...
image_raw_clock_adjust_double(
	...
	verbose(1, "doing clock adjustment (doubling values) on raw track %d", track);
	for (i = 0; i < size; i++)
		{
		d1 = data[i];
...
image_raw_clock_adjust_half(
	...
	verbose(1, "doing clock adjustment (halving values) on raw track %d", track);
	for (i = 0; i < size; i++)
		{
		d1 = data[i];
...
image_raw_read_track_data(
	...
	if (trk_hdr->magic != TRACK_MAGIC) error("wrong header magic in file '%s'", file_get_path(fil));
...
image_raw_read_track_text(
	...
	do
		{
		if (image_raw_text_token(&img_raw->txt, fil, token, GLOBAL_MAX_NAME_SIZE) == 0) return (0);
		if (string_equal(token, "track_data_hex")) hex_only = CW_BOOL_TRUE;
		else hex_only = CW_BOOL_FALSE;
		if (! string_equal2(token, "track_data", "track_data_hex")) image_raw_text_error(&img_raw->txt, fil, "directive expected");
		trk_hdr->magic = TRACK_MAGIC;
		trk_hdr->track = image_raw_text_number(
			&img_raw->txt,
			fil,
			NULL,
			0,
			CW_BOOL_FALSE);
		trk_hdr->clock = image_raw_text_number(
			&img_raw->txt,
			fil,
			NULL,
			0,
			CW_BOOL_FALSE);
		trk_hdr->flags = image_raw_text_number(
			&img_raw->txt,
			fil,
			NULL,
			0,
			CW_BOOL_FALSE);
		image_raw_text_token(&img_raw->txt, fil, token, GLOBAL_MAX_NAME_SIZE);
		if (! string_equal(token, "{")) image_raw_text_error(&img_raw->txt, fil, "{ expected");
		size = 0;
		while (1)
			{
			if (image_raw_text_token(&img_raw->txt, fil, token, GLOBAL_MAX_NAME_SIZE) == 0) image_raw_text_error(&img_raw->txt, fil, "} expected");
			if (string_equal(token, "}")) break;
			if (size > limit) error("track %d too large in file '%s'", trk_hdr->track, file_get_path(fil));
			data[size++] = image_raw_text_number(
				&img_raw->txt,
				fil,
				token,
				GLOBAL_MAX_NAME_SIZE,
	 ...
	 * so it would be ok to use img_raw->type here.
	 */
	...
	if (type == TYPE_REGULAR)
		...
		ofs = img_raw->txt.limit - img_raw->txt.ofs;
		ofs = file_seek(fil, -1, FILE_FLAG_NONE) - ofs;
		file_seek(fil, ofs, FILE_FLAG_NONE);
		img_raw->txt.ofs   = 0;
		img_raw->txt.limit = 0;
		}
...
image_raw_read_track2(
	...
	fifo_reset(ffo);
	if (img_raw->subtype == SUBTYPE_DATA) size = image_raw_read_track_data(img_raw, fil, trk_hdr, ffo);
	else size = image_raw_read_track_text(img_raw, fil, trk_hdr, ffo, type);
	if (size == 0) return (0);

	if (trk_hdr->track >= GLOBAL_NR_TRACKS) error("invalid track in file '%s'", file_get_path(fil));
	if (trk_hdr->clock >= CW_NR_CLOCKS) error("invalid clock in file '%s'", file_get_path(fil));
...
image_raw_hint_store(
	...
	if (img_raw->hints >= IMAGE_RAW_NR_HINTS) error("file '%s' has too many tracks", file_get_path(&img_raw->fil[0]));
...
image_raw_hint_search(
	...
	for (h = 0; h < img_raw->hints; h++)
		...
		return (image_raw_read_track2(img_raw, &img_raw->fil[file], img_trk, &trk_hdr, ffo, TYPE_REGULAR));
		}
...
image_raw_read_track(
	...
	while (1)
		...
		size = image_raw_read_track2(img_raw, &img_raw->fil[0], img_trk, &trk_hdr, ffo, img_raw->type);
...
image_raw_open(
	...
	if (file_is_readable(&img->raw.fil[0]))
		...
		for (i = 0; i < MAGIC_SIZE; i++)
			...
			error("file '%s' has wrong magic", file_get_path(&img->raw.fil[0]));
			}
		if (img->raw.subtype == SUBTYPE_TEXT)
			{
			subtype_name = " (text)";
			img->raw.txt = (struct image_raw_text)
				{
				.text  = malloc(TEXT_BUFFER_SIZE * sizeof (cw_char_t)),
				.size  = TEXT_BUFFER_SIZE,
				};
			if (img->raw.txt.text == NULL) error_oom();

			/*
			 * seek back to file start. needed for proper offset
			 * calculation in image_raw_read_track_text()
			 */

			file_seek(&img->raw.fil[0], 0, FILE_FLAG_NONE);
			}
		}
	else file_write(&img->raw.fil[0], magic_data3, MAGIC_SIZE);
done:
	verbose(1, "assuming '%s' is a %s%s", file_get_path(&img->raw.fil[0]), type_name, subtype_name);
	return (1);
	}
...
image_raw_close(
	...
	if ((file_is_readable(&img->raw.fil[0])) && (img->raw.type == TYPE_PIPE))
		{
		while (image_raw_read_track2(&img->raw, &img->raw.fil[0], NULL, &trk_hdr, &ffo, img->raw.type) > 0) ;
