...
cwtool_get_line(
	...
	if ((string_length(info) == 0) || (verbose_level < 0)) string_snprintf(line, size, "%s", name);
...
cwtool_get_disk(
	...
	if (dsk == NULL) error("unknown disk name '%s'", cmdline_get_disk_name());
	return (dsk);
	}
...
cwtool_info_print(
	...
	else fprintf(info_fh, "\r%s%s%s", line, &space[i], end);
...
cwtool_dump(
	...
	/* assuming first character in config_default is \n */

	printf("# builtin default config (" GLOBAL_VERSION_STRING ")\n%s", &config_default[1]);
	}
